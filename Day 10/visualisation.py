...
VISITED_COLOUR = pygrille.random_colour()


s_directions = []


def part1(data, visual_grid):
    data = data.split("\n")
    start = None
    ...
    for cell in visited:
        visual_grid[cell[0]][cell[1]].colour = VISITED_COLOUR
    done = False
    steps = 0
    while not done:
        visual_grid.check_open()
        visual_grid.draw()
        visual_grid.tick()
        steps += 1
        for pos_num, pos in enumerate(current_pos):
            ...
            for directions in PIPES[pipe_type]:
                ...
                if new_location not in visited:
                    visual_grid[new_location[0]][new_location[1]].colour = VISITED_COLOUR
...
def main():
    ...
    data = get_data()
    visual_grid = pygrille.Grid(50, [len(data.split("\n")),len(data.split("\n")[0])], framerate=5)
    part1_answer, visited = part1(data, visual_grid)
